...
namespace UploadLib.Services
{
    public class PictureAppService : IPictureAppService
    {
        ...
        public async Task ResizePic(List<FileInfo> browserFiles)
        {
            ...
            foreach (var imageFile in browserFiles)
            {
                ...
                Console.WriteLine($"Began {imageFile.Name}");
                var stream = imageFile.Open(FileMode.Open);
                ...
                Console.WriteLine($"End {imageFile.Name}");
            }
        }
